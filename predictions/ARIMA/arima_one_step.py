...
def fit_best_sarima_model(y_train, tmpdir, include_lags):
    ...
    # Save the best model
    model_save_path = os.path.join(tmpdir, f"best_sarima_model_{'with' if include_lags else 'without'}_lags.pkl")
...
def copy_files_to_results_directory(tmpdir, results_dir, include_lags):
    output_files = [
        ...
        os.path.join(tmpdir, f"best_sarima_model_{'with' if include_lags else 'without'}_lags.pkl")  
    ]
...
def main():
    ...
    if os.path.exists(model_save_path):
        ...
        best_sarima_model = fit_best_sarima_model(y[y.index < START_DATE], TMPDIR, INCLUDE_LAGS)
