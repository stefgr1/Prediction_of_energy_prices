...
class DWDScraper:
    """A class for scraping weather data from the German Weather Service (DWD)."""

    def __init__(self):
        config = load_config()
        self.data_dir = config['data']['dwd']
        self.bounding_boxes = config['bboxes']['germany']
    ...
    def get_all_stations(self, name, min_datum, max_datum):
        ...
        r = requests.get(
            self.url_hist + 'TU_Stundenwerte_Beschreibung_Stationen.txt')
        ...
        stations['von_datum'] = pd.to_datetime(
            stations['von_datum'], format='%Y%m%d')
        stations['bis_datum'] = pd.to_datetime(
            stations['bis_datum'], format='%Y%m%d')
        stations = stations[(stations['von_datum'] <= min_datum) & (
            stations['bis_datum'] >= max_datum)]
    ...
    def scrape_hist(self, name, start_date, end_date, stations_id):
        ...
        reg = 'stundenwerte_TU_' + j_stations_id + \
            '_[0-9]{8}_20221231_hist.zip'
        ...
        for i in range(len(hrefs_final)):
            ...
            file = glob.glob(filename.replace(
                '.zip', '') + '/produkt_tu_stunde*')
            station_data = pd.read_csv(file[0], sep=';')[
                ["STATIONS_ID", "MESS_DATUM", "TT_TU", "RF_TU"]]
            station_data['MESS_DATUM'] = pd.to_datetime(
                station_data['MESS_DATUM'], format='%Y%m%d%H')
            station_data = station_data.merge(
                station_filter, how="right", on='MESS_DATUM')
    ...
    def scrape_recent(self, name, end_date, stations_id):
        ...
        start_date = datetime.strptime(
            str(current_year) + '-01-01', "%Y-%m-%d")
        end_date = datetime.strptime(
            str(current_year) + '-' + end_date, "%Y-%m-%d")
        dates = pd.date_range(start_date, end_date, freq='H')
