...
def load_data(year, path):
    ...
    # Load the data
    file_path = path / f"SOLAR_data_{year}.csv"
...
dataframes = [load_data(
    year, config["data"]["solar_input"]) for year in years]
...
df_merged.rename(columns={'Datum': 'Month', 'value': 'SolarRadiation',
...
daily_average_solar = df.groupby(
    'Date')['SolarRadiation'].mean().reset_index(name='AverageSolarRadiation')

# round all values to 2 decimal places
daily_average_solar = daily_average_solar.round(2)

print(daily_average_solar)

# %% Save the daily wind data
output_file_name = f"temp_data_daily_{years[0]}_{years[-1]}.csv"
output_path = Path(config["data"]["solar_output"]) / output_file_name
save_data_frame(daily_average_solar, output_path)
